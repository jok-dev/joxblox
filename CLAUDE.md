...
- Use `./build.sh` (not `go build` directly) when building the app or verifying that a change compiles end-to-end. The shipped binary depends on the bundled Rust extractor, the mesh-renderer side binary, and embedded Windows resources — `go build ./cmd/joxblox` alone produces a Go-only binary that's missing those payloads, which can mask runtime regressions even when `go build` succeeds. `./build.sh` (with no args, or `./build.sh all`) builds every component; pass `go`, `rust`, or `mesh-renderer` for one-off rebuilds when only that piece changed.

...
func BindMainFileMenu(
	...
	scanFileActions scan.ScanTabFileActionsProvider,
	allScanFileActions []scan.ScanTabFileActionsProvider,
	selectScanContext func(string),
	showSettings func(fyne.Window),
	buildHelpMenu func(fyne.Window) *fyne.Menu,
) {
	...
	rebuildMainMenu = func() {
		...
		if buildHelpMenu != nil {
			helpMenu = buildHelpMenu(window)
		}
		if helpMenu == nil {
...
func buildRecentFilesMenuItem(
	...
	scanTab *container.TabItem,
	selectScanContext func(string),
	providers []scan.ScanTabFileActionsProvider,
) *fyne.MenuItem {
...
func collectScanFileActions(providers []scan.ScanTabFileActionsProvider) []*scan.ScanTabFileActions {
	actionsByContextKey := map[string]*scan.ScanTabFileActions{}
	collectedActions := make([]*scan.ScanTabFileActions, 0, len(providers))
...
func snapshotScanWorkspace(collectedActions []*scan.ScanTabFileActions) map[string][]loader.ScanResult {
...
func clearAllScanResults(window fyne.Window, providers []scan.ScanTabFileActionsProvider) {
...
func saveAllScanResults(window fyne.Window, providers []scan.ScanTabFileActionsProvider) {
	collectedActions := collectScanFileActions(providers)
	if len(collectedActions) == 0 {
	...
	go func() {
		payloadBytes, marshalErr := scan.MarshalScanWorkspace(tablesByContext, serializeProgress)
		if marshalErr != nil {
			progress.Hide()
			fyne.Do(func() {
		...
		if writeErr := scan.WriteFileWithProgress(selectedExportPath, payloadBytes, writeProgress); writeErr != nil {
			progress.Hide()
			fyne.Do(func() {
...
func loadAllScanResultsFromPathWithActionsAsync(
	window fyne.Window,
	collectedActions []*scan.ScanTabFileActions,
	importPath string,
	onComplete func(string, bool),
) {
	...
	go func() {
		payloadBytes, readErr := scan.ReadFileWithProgress(importPath, readProgress)
		if readErr != nil {
			progress.Hide()
			fyne.Do(func() {
		...
		tablesByContext, parseErr := scan.UnmarshalScanWorkspace(payloadBytes, parseProgress)
		if parseErr != nil {
			progress.Hide()
			fyne.Do(func() {
...
func collectRecentFiles(providers []scan.ScanTabFileActionsProvider) []string {
	collectedActions := collectScanFileActions(providers)
...
func switchToContextTab(
	...
	case scan.ScanContextFolder, scan.ScanContextFolderDiff, scan.ScanContextRBXLSingle, scan.ScanContextRBXLDiff:
		if scanTab != nil {
			tabs.Select(scanTab)
		}

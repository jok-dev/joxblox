...
func NewModelHeatmapTab(window fyne.Window) fyne.CanvasObject {
	...
	backgroundSelect := widget.NewSelect([]string{ui.ExpandedBackgroundBlack, ui.ExpandedBackgroundWhite}, nil)
	backgroundSelect.SetSelected(ui.ExpandedBackgroundBlack)
	backgroundSelect.OnChanged = func(mode string) {
		viewer.SetBackground(ui.ZoomPanBackgroundColor(mode))
	}
	viewer.SetBackground(ui.ZoomPanBackgroundColor(backgroundSelect.Selected))
	...
	browseButton := widget.NewButtonWithIcon("Choose File", theme.FolderOpenIcon(), func() {
		common.PickRBXLSource(window, loadModelHeatmap, func(err error) {
...
func buildModelHeatmapInstances(mapParts []extractor.MapRenderPartResult, refs []extractor.PositionedResult) []modelHeatmapMeshInstance {
	...
	for _, ref := range refs {
		...
		switch {
		case report.IsMeshContentProperty(propertyName):
			meshRefsByPath[instancePath] = append(meshRefsByPath[instancePath], reference)
		case report.IsTextureContentProperty(propertyName),
			report.IsSurfaceAppearanceProperty(propertyName, originalInstanceType):

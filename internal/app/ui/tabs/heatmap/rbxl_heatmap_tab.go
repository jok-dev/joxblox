...
const (
	...
	rbxlHeatmapDefaultBgColor       = ui.ExpandedBackgroundBlack
...
func NewRBXLHeatmapTab(window fyne.Window) (fyne.CanvasObject, func(string)) {
	...
	heatmapViewer := ui.NewZoomPanImage(currentOption)
	...
	saveButton := widget.NewButtonWithIcon("Save Heatmap", theme.DownloadIcon(), func() {
		...
		saved, saveErr := ui.SaveBytesWithNativeDialog("Save Heatmap", currentOption.FileName, currentOption.Bytes)
		if saveErr != nil {
			fyne.Do(func() {
	...
	selectButton := widget.NewButton("Select .rbxl/.rbxm File", func() {
		common.PickRBXLSource(window, func(filePath string) {
	...
	buildButton.OnTapped = func() {
		...
		pathPrefixes := []string{}
		if pathFilterEnabled {
			pathPrefixes = common.WhitelistPatternsToPathPrefixes(pathFilterText)
		}
...
func renderRBXLHeatmapScene(scene *RBXLHeatmapScene, backgroundImageBytes []byte, heatOpacity float64, heatSpread float64, heatMetric rbxlHeatMetric, showGrid bool) (ui.PreviewDownloadOption, error) {
	imageBytes, renderErr := renderRBXLHeatmapImage(scene, backgroundImageBytes, heatOpacity, heatSpread, heatMetric, showGrid)
	if renderErr != nil {
		return ui.PreviewDownloadOption{}, renderErr
	}
	...
	return ui.PreviewDownloadOption{
		LabelText: labelText,
		FileName:  fileName,
		Bytes:     imageBytes,
...
func openHeatmapSquareAssetsWindow(parentWindow fyne.Window, scene *RBXLHeatmapScene, cell heatmap.Cell, rows []heatmapSquareAssetRow, heatMetric rbxlHeatMetric) {
	...
	summaryLabel.Wrapping = fyne.TextWrapWord
	explorer := scan.NewScanResultsExplorer(detailWindow, scan.ScanResultsExplorerOptions{
		Variant:            scan.ScanResultsExplorerVariantHeatmap,
...
func blankHeatmapPreviewOption() ui.PreviewDownloadOption {

...
func extractAssetIDsWithRustFromFile(filePath string, limit int, stopChannel <-chan struct{}) ([]int64, string, error) {
	...
	assetIDsFromDOM := extractAssetIDsFromRustDOMJSON(commandOutputText, limit)
	logDebugf(
		"Rust extractor returned JSON bytes=%d and parsed IDs=%d",
		len(commandOutput),
		len(assetIDsFromDOM),
	)
	return assetIDsFromDOM, commandOutputText, nil
}
...
func extractAssetIDsFromRustDOMJSON(domJSON string, limit int) []int64 {
	var extractorResult struct {

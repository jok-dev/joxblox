...
const scanTableExportVersion = 1
...
func marshalScanTable(rows []scanResult) ([]byte, error) {
	...
	for _, row := range rows {
		...
		exportRows = append(exportRows, scanTableExportRow{
			AssetID:              row.AssetID,
...
func unmarshalScanTable(payloadBytes []byte) ([]scanResult, error) {
	...
	for _, row := range payload.Rows {
		...
		if strings.TrimSpace(row.ImageBytesBase64) != "" {
			...
			if decodeErr != nil {
				return nil, fmt.Errorf("failed decoding image bytes for asset %d: %w", row.AssetID, decodeErr)
			}
		...
		importedRows = append(importedRows, scanResult{
			AssetID:              row.AssetID,
			UseCount:             row.UseCount,
			FilePath:             row.FilePath,
